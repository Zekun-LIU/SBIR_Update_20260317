...
Uses the official Congress.gov API (no scraping, no 403 errors).
"""
...
def fetch_bill_data() -> dict:
    ...
    if not CONGRESS_KEY:
        ...
        result["status_text"] = "Config Error"
        result["latest_action"] = "CONGRESS_API_KEY secret is missing."
        return result

    headers = {"X-API-Key": CONGRESS_KEY}
    bill_endpoint = f"{API_BASE}/bill/{CONGRESS}/{BILL_TYPE}/{BILL_NUMBER}"

    try:
        resp = requests.get(bill_endpoint, headers=headers, timeout=20)
        resp.raise_for_status()
        data = resp.json().get("bill", {})

        latest = data.get("latestAction", {})
        result["latest_action"]      = latest.get("text", "No action text available")
        result["latest_action_date"] = latest.get("actionDate", "Unknown")
        result["raw_status"]         = result["latest_action"]
        log.info("API fetch OK | action: %s | date: %s",
                 result["latest_action"][:80], result["latest_action_date"])
    except Exception as exc:
        log.error("Bill API fetch failed: %s", exc)
    ...
    try:
        actions_resp = requests.get(
            f"{bill_endpoint}/actions",
            headers=headers,
            params={"limit": 10, "sort": "actionDate+desc"},
            timeout=20,
        )
        actions_resp.raise_for_status()
        actions = actions_resp.json().get("actions", [])
        combined = " ".join(a.get("text", "") for a in actions)
        result["raw_status"] = combined or result["raw_status"]
        log.info("Fetched %d recent actions", len(actions))
    except Exception as exc:
        log.warning("Actions fetch failed (using latestAction only): %s", exc)
...
def generate_summary(bill_data: dict, changed: bool) -> str:
    ...
    if not api_key:
        ...
        return (
            ...
            f"Latest action: {bill_data['latest_action'][:150]}. "
            f"{change_note}"
        )
    prompt = (
        "You are a concise legislative analyst. Write 1-3 sentences summarizing "
        "the current status of S.3971, the SBIR/STTR Reauthorization Act.\n\n"
    ...
    try:
        ...
        return f"S.3971 status: {bill_data['status_text']}. Latest: {bill_data['latest_action'][:150]}."


# ── Email ──────────────────────────────────────────────────────────────────────

def build_email_html(bill_data, changed, prev_status, summary):
    ...
    prev_row = (
        f'<tr><td style="color:#6b7280;padding:8px 0;width:160px;">Previous Status</td>'
        f'<td style="padding:8px 0 8px 16px;">{prev_status or "N/A"}</td></tr>'
        if prev_status else ""
    )
...
<body style="font-family:Georgia,serif;max-width:620px;margin:0 auto;padding:24px;background:#f9f9f7;color:#1a1a1a;">
  ...
  <div style="background:#fff;padding:24px 28px;border:1px solid #e5e5e3;border-top:none;">
    ...
    <div style="margin-top:24px;">
      <a href="{BILL_URL}" style="display:inline-block;background:#1e3a5f;color:#fff;text-decoration:none;padding:10px 20px;border-radius:6px;font-size:14px;">View on Congress.gov →</a>
    </div>
  </div>
...
def run():
    ...
    if changed or is_critical or force_daily:
        ...
        send_email(subject, build_email_html(bill_data, changed, prev_status, summary))
    else:
        log.info("No change — skipping email.")
